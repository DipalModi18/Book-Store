...
public class ViewBooks extends JPanel implements ActionListener, WindowListener
{
	...
	ViewBooks()
	{
		...
		//jbInsert.setBounds(300, 300, 100, 50);
		//jbCancel.setBounds(350, 300, 100, 20);
	...
	{
		if(jtBookId.getText().equals("") && jtBookname.getText().equals("") && jtPublication.getText().equals("") && jtAuthor.getText().equals(""))
		{
			//books=Book.booksRecords();
			if(books.size()>0)
			{
		...
		if(countAuthors==1)
		{
				...
				while(i.hasNext())
				{
					...
					for(int j=0;j<3;j++)
					{
						System.out.println("bb.author["+j+"] :"+bb.author[j]+" jtauthor[0] :"+jtAuthor[0].getText());
